...
def ProcessEachImgByRotation(srcImgDir, srcImgList, dstImgPath):
	'''
	...
	gt_file = open(srcImgDir + srcImgList)
	...
	for i in range(len(lines)):
		...
		curFileName = os.path.join(srcImgDir, curline.split(" ")[ 0 ])
...
def CropImgWithAngle(oldcenter, newcenter, angle, imgName, orgImg, newImg, srcImgDir, dstImgPath):
	...
	for i in range(len(lines)):
		...
		imgFileName = os.path.splitext(os.path.basename(imgName))[0] + '_angle_{0}_idx_{1}'.format(angle, i) + ".jpg"

		img_crop.save(os.path.join(dstImgPath, imgFileName))
		SaveNewAnnotation(imgFileName, content, dstImgPath)	
		# print "Saving crop img: " + os.path.splitext(os.path.basename(imgName))[0] + '_angle_{0}'.format(angle) + ".jpg"


def SaveNewAnnotation(imgFileName, imgLabel, dstImgPath):
	dstImgList = os.path.join(dstImgPath, "filelist.txt")

	if os.path.exists(dstImgList):
		writer = open(dstImgList, 'w')
	else:
		writer = open(dstImgList, 'a')
	writer.write(imgFileName + " " + imgLabel + "\n")
...
if __name__ == '__main__': 
    ...
    srcImgList = sys.argv[2]
    dstImgPath = sys.argv[3]
    ...
    ProcessEachImgByRotation(srcImgDir, srcImgList, dstImgPath)
